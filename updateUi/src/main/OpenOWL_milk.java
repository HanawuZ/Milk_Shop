...
class OpenOWL_milk {
   ...
   //Connexion
     static OntModel OpenMilkConnectOWL(){
        
        OntModel mode = null;
        mode = ModelFactory.createOntologyModel( OntModelSpec.OWL_MEM_RULE_INF );
        java.io.InputStream in = FileManager.get().open( "./src/UI/milk.owl" ); // be sure file into c:\
        if (in == null) {
     ...
     static  org.apache.jena.query.ResultSet ExecSparQlMilk(String Query){
         
          org.apache.jena.query.Query query = QueryFactory.create(Query);
                QueryExecution qe = QueryExecutionFactory.create(query, OpenMilkConnectOWL());
     ...
     // Connecté au OWL File et retourner le String
      static  String GetResultAsString(String Query){
        try {
            ...
            Logger.getLogger(OpenOWL_milk.class.getName()).log(Level.SEVERE, null, ex);
        }
         return s;
      }
